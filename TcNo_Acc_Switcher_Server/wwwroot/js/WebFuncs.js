...
function copyToClipboard(str) {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "CopyToClipboard", str);
}
...
async function forget(e) {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `Get${getCurrentPage()}ForgetAcc`).then((r) => {
...
async function restoreSteamAccounts() {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "Steam_RestoreSelected", reqSteamIds).then((r) => {
        if (r === true) {
...
async function restoreBattleNetAccounts() {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "BattleNet_RestoreSelected", reqBattleNetId).then((r) => {
        if (r === true) {
...
function copy(request, e) {
    ...
    function steam() {
        ...
        switch (request) {
            ...
            case "SteamId3":
            case "SteamId":
                DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "CopySteamIdType", request, steamId64);
                break;
...
function swapTo(request, e) {
    ...
    if (request === -1) DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `SwapTo${getCurrentPage()}`, selected.attr("id")); // -1 is for undefined.
    else DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `SwapTo${getCurrentPage()}WithReq`, selected.attr("id"), request);
}
...
function openUserdata(e) {
    ...
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `SteamOpenUserdata`, selected.attr("id"));
}
...
function createShortcut(args = '') {
    ...
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "CreateShortcut", getCurrentPage(), accId, selected.attr("Username"), args);
}
...
function refreshUsername() {
    ...
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "UbisoftRefreshUsername", selected.attr("id"));
}


// NEW LOGIN
function newLogin() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `NewLogin_${getCurrentPage()}`);
}

function hidePlatform() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "HidePlatform", selectedElem);
}

function createPlatformShortcut() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiCreatePlatformShortcut", selectedElem);
}
...
async function exportAllAccounts() {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiExportAccountList", selectedElem).then((r) => {
...
async function currentDiscordLogin() {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "SkipGetUsername").then((r) => {
	    skipQuestion = r;
    });
    _ = await promise;
...
function currentUbisoftLogin() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "UbisoftAddCurrent");
}
...
function OpenLinkInBrowser(link) {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "OpenLinkInBrowser", link);
}
...
async function showModal(modaltype) {
    ...
    if (modaltype === "info") {
        ...
        currentVersion = "";
        DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiGetVersion").then((r) => {
            currentVersion = r;
            $("#modal_contents").append(`<div class="infoWindow">
                ...
                <div class="rightContent">
                    ...
                    <div class="linksList">
                        <a onclick="OpenLinkInBrowser('https://github.com/TcNobo/TcNo_Acc_Switcher');"><svg viewBox="0 0 24 24" draggable="false" alt="GitHub" class="modalIcoGitHub"><use href="img/icons/ico_github.svg#icoGitHub"></use></svg>View on GitHub</a>
                        ...
                        <a onclick="OpenLinkInBrowser('https://github.com/TcNobo/TcNo_Acc_Switcher/blob/master/DISCLAIMER.md');"><svg viewBox="0 0 2084 2084" draggable="false" alt="GitHub" class="modalIcoDoc"><use href="img/icons/ico_doc.svg#icoDoc"></use></svg>Disclaimer</a>
                    </div>
                </div>
...
async function showPasswordModal() {
    ...
    let dPromise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", `GiCheckPlatformPassword`, platform).then((r) => {
        if (r === 1) {
    ...
    $("#modal_contents").append(`<div id="modal_contents">
	    ...
	    + `
			<p id="formNotice"></p>
	        <div class="settingsCol inputAndButton">
				<button class="modalOK extra" type="button" id="set_account_name" onclick="OpenLinkInBrowser('https://github.com/TcNobo/TcNo_Acc_Switcher/wiki/Platform:-Discord#why-a-password');"><span><svg viewBox="0 0 384 512" draggable="false" alt="C" class="footerIcoInline"><use href="img/fontawesome/question.svg#img"></use></svg></span></button>
...
function Modal_Finalise(platform, platformSettingsPath) {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiUpdatePath", platformSettingsPath, $("#FolderLocation").val());
    $(".modalBG").fadeOut();
    location.reload();
}
async function Modal_Confirm(action, value) {
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiConfirmAction", action, value).then((r) => {
...
function Modal_FinaliseAccString(platform) {
    ...
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", platform + "AddCurrent", name);
    $(".modalBG").fadeOut();
    $("#acc_list").click();
}
...
function Modal_FinaliseAccNameChange() {
	...
	let name = (raw.indexOf("TCNO:") === -1 ? raw.replace(/[<>: \.\"\/\\|?*]/g, "-") : raw); // Clean string if not a command string.
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "ChangeUsername", $(".acc:checked").attr("id"), name, getCurrentPage());
}
...
async function Modal_HandlePassword() {
    ...
    var promise = DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "GiVerifyPlatformPassword", getCurrentPage(), pass).then((r) => {
        if (!r)
	        window.notification.new({
		        type: "error",
...
function forgetBattleTag() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "DeleteUsername", $(".acc:checked").attr("id"));
}

function refetchRank() {
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "RefetchRank", $(".acc:checked").attr("id"));
}
...
if (streamerMode){
console.log.apply(console, ["%cTcNo Account Switcher%c: ERROR!\\nMake sure that Streamer Mode is disabled/not active when running this command!\\n%chttps://github.com/TcNobo/TcNo_Acc_Switcher/wiki/Platform:-Discord#adding-accounts-to-the-discord-switcher-list ", 'background: #290000; color: #F00','background: #290000; color: white','background: #222; color: lightblue']);
}else{
  ...
  console.log.apply(console, ["%cTcNo Account Switcher%c: Successfully copied information!\\nPaste it into the input box in the account switcher to update/set image and username.\\n%chttps://github.com/TcNobo/TcNo_Acc_Switcher/wiki/Platform:-Discord#adding-accounts-to-the-discord-switcher-list ", 'background: #222; color: #bada55','background: #222; color: white','background: #222; color: lightblue']);
}`;
    DotNet.invokeMethodAsync("TcNo_Acc_Switcher_Server", "CopyToClipboard", code);
    window.notification.new({
	    type: "success",
	    title: "Copied",
